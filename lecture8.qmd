...
format:
  revealjs:
    theme: clean.scss
    scrollable: true
    footer: "Lecture 8 - [Back to home](https://miguelfudolig-eab703-slides.share.connect.posit.cloud)"
    code-block-bg: true
...
This could also be calculated using the `eta_squared` function in the `effectsize` package in `R`.
...
eta_squared(mod_aov)
```
The p-value is <2e-16, which is less than 0.05. We reject the null hypothesis and therefore there is sufficient evidence that at least one species has a different population average sepal length compared to the others. The effect size is `r round(eta_squared(mod_aov)$Eta2,2)` 
...
eta_squared(mod_aov)
```
The p-value is <2e-16, which is less than 0.05. We reject the null hypothesis and therefore there is sufficient evidence that at least one species has a different population average bill length compared to the others. The effect size is `r round(eta_squared(mod_aov)$Eta2,2)` 
...
```
There is evidence of an interaction between treatment and family history ($\eta^2_p$=`r round(eta_squared(mod_aov)$Eta2[3],2)`,p=0.0002). Based on the calculated means, the difference between the treatments was higher for those with family history of hypertension compared to those without.
:::
...
There is no evidence of an interaction between Ascorbic Acid and Zinc ($\eta^2_p$=`r round(eta_squared(mod_aov)$Eta2[3],2)`, p=0.84). There is also no evidence of efficacy for Ascorbic Acid ($\eta^2_p$=`r round(eta_squared(mod_aov)$Eta2[1],2)`, p=0.67) and Zinc ($\eta^2_p$=`r round(eta_squared(mod_aov)$Eta2[2],2)`, p=0.83)
:::