...
format:
  revealjs:
    theme: clean.scss
    scrollable: true
    footer: "Lecture 7 - [Back to home](https://miguelfudolig-eab703-slides.share.connect.posit.cloud)"
    code-block-bg: true
...
df2 <- mutate(df, probs = 1-ppois(xc-1,lambda=6))
df2
```

The data frame shows that when $x_c=11$, $P(X \geq 11) = 0.0426$, which is lower than 0.05. This means the rejection/critical region is $X\geq 11$. If we get at least 10 cases, we have evidence of an outbreak. If we get less than 10 cases, we have no evidence of an outbreak.
:::
...
Specifically, for $H_a: \mu < \mu_0$, the rejection region is $z < z_\alpha$.  Since we typically choose a low value for $\alpha$, we expect a negative value for $z_\alpha$. In `R`,
...
For $H_a: \mu > \mu_0$, the rejection region is $z > z_{1-\alpha}$.
...
```{r}
#| eval: false
pnorm(-abs(z),mean=0,sd=1) + pnorm(abs(z),mean=0,sd=1,lower.tail=F)

# OR 

2*pnorm(abs(z),mean=0,sd=1,lower.tail=F)
...
```{r}
pval <- pnorm(-abs(z),mean=0,sd=1) + pnorm(abs(z),mean=0,sd=1,lower.tail=F)
pval
...
-   $H_a: \pi < \pi_0 \to P(Z < z)$ or `pnorm(z,mean=0,sd=1)`
-   $H_a: \pi > \pi_0 \to P(Z > z)$ or `pnorm(z,mean=0,sd=1,lower.tail=F)`
-   $H_a: \pi \neq \pi_0 \to P(Z < -|z|) + P(Z>|z|)$ or `2*pnorm(abs(z),mean=0,sd=1,lower.tail=F)`
