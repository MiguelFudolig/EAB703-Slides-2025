...
Suppose the true population/long-run proportion is $p$. Invoking the Central Limit Theorem, the sampling distribution of the sample proportion, $\hat{P}$ when the sample size is large follows a Gaussian distribution with mean $p$ and variance $\frac{p(1-p)}{n}$.
...
It follows that the standard error of the sample proportion is $\sigma_{\hat{p}} = \sqrt{\frac{p(1-p)}{n}}$.

## Standardization

The sample proportion can also be standardized based on the properties of the sampling distribution. The sample mean $\hat{p}$ can be transformed into a random variable $Z$ such that $Z \sim N(0,1)$.
...
$$
P(\hat{P} \leq \hat{p}) = P\left(Z \leq \frac{\hat{p}-p}{\sqrt{\frac{p(1-p)}{n}}}\right) = P(Z \leq z)
$$
