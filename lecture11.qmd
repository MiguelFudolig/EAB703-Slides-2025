...
format:
  revealjs:
    theme: clean.scss
    scrollable: true
    footer: "Lecture 11 - [Back to home](https://miguelfudolig-eab703-slides.share.connect.posit.cloud)"
    code-block-bg: true
...
Use the Wilcoxon Signed-Rank Test to test whether the cortisol levels decreased after singing.
...
```{r}
wtest <- wilcox.test(x=example$before,y=example$after,paired=T,alternative="greater",correct=TRUE)
wtest
```
The resulting Wilcoxon test p-value is `r wtest$p.value`. We fail to reject the null hypothesis at the significance level of 0.05. We have insufficient evidence to claim that the cortisol levels decreased after singing.
...
$$
H_0: \tilde{\mu}_d=0;H_a: \tilde{\mu}_d\neq 0
$$
